...
void game_loop(t_game_state *game_state)
{
	while ( true )
	{
		//SDL_SetRenderDrawColor( game_state->render_state.renderer, 0, 0, 0, SDL_ALPHA_OPAQUE );
		//SDL_RenderClear( game_state->render_state.renderer );
		SDL_SetRenderDrawColor( game_state->render_state.renderer, 0, 0, 255, SDL_ALPHA_OPAQUE );
		short i = 0;
		while ( i < game_state->render_state.w )
		{
			SDL_RenderDrawLine( game_state->render_state.renderer, i, 100, i, 300 );
			i++;
		}
